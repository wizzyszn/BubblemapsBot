...
const decentralizationScore = async (ctx: Context) => {
  ...
  if (!supportedChains.find((supportedChain) => supportedChain === chain)) {
    return ctx.reply(
      "❌Unsupported chain. Please use a valid chain shorthand.\n\nUsage: /dexscore <chain> <token_address>"
    );
  }
  ...
  try {
    const response = await fetch(
      `https://api-legacy.bubblemaps.io/map-metadata?chain=${chain}&token=${tokenAddress}`
    );
    if (!response.ok) {
    ...
    await ctx.reply(
      "An error occurred while fetching the Decentralization score for this token. Please try again later or check the coin name"
    );
  }
};

export default decentralizationScore;
