 ...
 */
export async function isBubbleMapAvailable(chain: string, token: string): Promise<boolean> {
  const res = await axios.get('https://api-legacy.bubblemaps.io/map-availability', {
    params: { chain, token },
  });
...
export async function generateBubbleMapScreenshot(chain: string, token: string): Promise<string> {
  ...
  try {
    ...
    // Take screenshot
    const screenshotPath = path.join(__dirname, 'bubblemap.png');
