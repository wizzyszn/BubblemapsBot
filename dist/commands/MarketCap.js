...
const marketCap = async (ctx) => {
    ...
    try {
        ...
        if (!coins || coins.length === 0) {
            return ctx.reply(`No cryptocurrency found for "${coinInput}". Try another name or symbol.`);
        }
...
24h Volume: $${volume}
    `;
        ...
        await ctx.reply("An error occurred while fetching market cap. Please try again later or check the coin name.");
    }
};
